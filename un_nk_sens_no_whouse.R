# code/un_nk_sens_no_whouse.R — UN_NK sensitivity: drop W_House_Cons_Share
source("code/utils.R")
...
# 4) فقط W_Inflation (بدون W_House_Cons_Share)
nk <- add_W_by_year(nk, Wlist, vars = c("Inflation"), year_var = "Year_num")
...
# 6) فرمول حساسیت (حذف W_House_Cons_Share)
rhs <- c(
  "Lag_Unemployment",
  "House_Cons_Share", "W_Inflation",
  "Economic_Participation_Rate","Industry_Share_of_GDP","Natural_Disasters",
  ofe_cols
)
fmla_un_nk_noWH <- as.formula(paste("Unemployment ~", paste(rhs, collapse = " + ")))
...
print(summary(fit_un_nk_noWH))
save_model_outputs(fit_un_nk_noWH, "UN_NK_sens_no_WHOUSE")