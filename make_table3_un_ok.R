# code/make_table3_un_ok.R — Table 3: UN_OK (ضرایب اصلی)
source("code/utils.R")

path_coef <- "outputs/article/models/UN_OK_main_coef_utf8.csv"
stopifnot(file.exists(path_coef))
df <- readr::read_csv(path_coef, show_col_types = FALSE)

# فقط متغیرهای اصلی (OFE حذف)
keep <- c("Lag_Unemployment","Inflation","W_Inflation",
...
# ذخیره
out_csv  <- "outputs/article/tables/Table3_UN_OK.csv"
out_xlsx <- "outputs/article/tables/Table3_UN_OK.xlsx"
readr::write_excel_csv(tbl3, out_csv)
writexl::write_xlsx(tbl3, out_xlsx)
cat("Saved Table 3 to:\n -", out_csv, "\n -", out_xlsx, "\n")