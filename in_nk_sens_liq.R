# code/in_nk_sens_liq.R — IN_NK sensitivity: Interest → Liquidity
source("code/utils.R")
...
# 6) فرمول حساسیت (Liquidity به‌جای Interest)
rhs_liq <- c(
...
fmla_in_nk_liq <- as.formula(paste("Inflation ~", paste(rhs_liq, collapse = " + ")))
...
print(summary(fit_in_nk_liq))
save_model_outputs(fit_in_nk_liq, "IN_NK_sens_Liq")